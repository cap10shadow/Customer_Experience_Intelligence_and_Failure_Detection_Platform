...
async def handle_business_impact_completed(
    ...
    this repository yet. This route contains no logic of its own: it
    reads the raw JSON body exactly as an untyped broker message payload
    would arrive (deliberately not FastAPI/Pydantic-validated -- that
    deserialization is `BusinessImpactCompletedConsumer`'s own
    Infrastructure responsibility, not this route's) and hands it directly
    to the Consumer.

    Not mounted under `/api/v1`: this is an internal execution-lifecycle
    trigger, not part of the public, read-only Evaluation API.
