...
class RegionDetector:
    ...
    async def detect(self, current: TrendWindow, previous: TrendWindow, dataset_id: uuid.UUID) -> List[AnomalyCandidate]:
        current_rows = await self.repository.count_complaints_by_region(current.start, current.end, dataset_id)
        previous_rows = await self.repository.count_complaints_by_region(previous.start, previous.end, dataset_id)
