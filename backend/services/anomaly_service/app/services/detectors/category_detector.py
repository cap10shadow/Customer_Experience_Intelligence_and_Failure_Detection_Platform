...
class CategoryDetector:
    ...
    async def detect(self, current: TrendWindow, previous: TrendWindow, dataset_id: uuid.UUID) -> List[AnomalyCandidate]:
        current_rows = await self.repository.count_enrichments_by_category(current.start, current.end, dataset_id)
        previous_rows = await self.repository.count_enrichments_by_category(previous.start, previous.end, dataset_id)
